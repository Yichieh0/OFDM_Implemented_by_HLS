...
void Rx(hls::stream<T_in> FFT_in[SSR], hls::stream<ap_uint<pixl_bit>, FFT_LEN>& pic_out, hls::stream<ap_fixed<IN_WL,IN_IL>, FFT_LEN>& deQAM_output_real, hls::stream<ap_fixed<IN_WL,IN_IL>, FFT_LEN>& deQAM_output_imag, hls::stream<ap_uint<6>>& s_out, ap_uint<32> DATA_LEN, ap_uint<32> pilot_width, ap_uint<32> TAPS_NUM){
#pragma HLS dataflow
...
#pragma HLS array_partition variable=Equal_out type=complete
		data_temp.real(Equal_out[0].read());//Equal_out
		data_temp.imag(Equal_out[1].read());//Equal_out
		if((k%pilot_width)!=0){
      ...
      //------------------------------------------------------------//
			data_out_main(data_real,data_imag,pic_out,s_out, DATA_LEN);
		}
	}
