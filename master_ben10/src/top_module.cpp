...
#pragma HLS dataflow
  ...
  for(int w = 0; w<(int)DATA_LEN/((FFT_LEN-(FFT_LEN/pilot_width))/SSR); w++){
    Tx(in, channel_in_real, channel_in_imag, pilot_width);
    AWGN(channel_in_real, channel_in_imag, FFT_in, sqrt_No, TAPS_NUM, pilot_width);    
    Rx(FFT_in, pic_out, deQAM_output_real, deQAM_output_imag, pilot_width, TAPS_NUM);
  }
	return;
}
